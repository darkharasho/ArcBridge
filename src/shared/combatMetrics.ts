// Combat metrics derived from EI JSON fields and project-defined weighting rules.
import { Player } from './dpsReportTypes';
import { getPlayerBoonGenerationMs } from './boonGeneration';

export interface OutgoingCcGroup {
    category: string[];
    skills: { id: number; coefficient: number }[];
}

export const OUTGOING_CC_SKILLS: OutgoingCcGroup[] = [
    {
        category: ["Relics"],
        skills: [
...
// Mapping for incoming strips with per-skill weighting.
// These track boon strips received from enemies
export const INCOMING_STRIP_SKILL_WEIGHTS: { [id: number]: number } = {
    // 4x multiplier
...
// Mapping for incoming crowd control with per-skill weighting.
...
export const INCOMING_CC_SKILL_WEIGHTS: { [id: number]: number } = {
    // Elementalist
...
export function computeOutgoingCrowdControl(player: Player): number {
    ...
    for (const damageDistList of player.totalDamageDist) {
        ...
        for (const skill of damageDistList) {
            ...
            const mapEntry = OUTGOING_CC_SKILLS.find(m =>
...
export function computeIncomingDisruptions(player: Player): { strips: { total: number, missed: number, blocked: number }, cc: { total: number, missed: number, blocked: number } } {
    ...
    for (const damageTakenList of player.totalDamageTaken) {
        ...
        for (const hit of damageTakenList) {
            // Strips
            const stripMult = INCOMING_STRIP_SKILL_WEIGHTS[hit.id];
            if (stripMult) {
            ...
            // CC - apply per-skill weight to normalize multi-hit effects.
            const ccMult = INCOMING_CC_SKILL_WEIGHTS[hit.id];
            if (ccMult) {
...
export function computeDownContribution(player: Player): number {
    ...
    let total = 0;
    // Down contribution is aggregated across the target arrays.
