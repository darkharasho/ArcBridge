...
export function ingestLogFightDiffMode(log: any, idx: number) {
    ...
    return {
        ...
        shortLabel: `F${idx + 1}`,
        fullLabel: `${buildFightLabelV2({ zone: details?.fightName || log?.fightName || log?.encounterName || 'Unknown', avgPosition: computeFightAvgPosition(details) })} • ${formatDurationMs(durationMs)}`,
        mapName,
        timestamp,
...
export function computeFightDiffMode(sortedFightLogsWithDetails: Array<{ log: any; originalIndex: number }>) {
    return sortedFightLogsWithDetails
        .map(({ log }, idx) => ingestLogFightDiffMode(log, idx))
        .filter(Boolean);
}
