/**
 * The pre-migration, Elite-Insights-shaped `computeOutgoingConditions`,
 * preserved verbatim from `packages/bridge-metrics/src/conditionsMetrics.ts`
 * at d199c1bc (the commit before unit 5b).
 *
 * It exists only so the unit 5b oracle has something to compare the native
 * implementation against. It is test-only, never exported from the package,
 * and is deleted at Step N with the rest of the EI path.
 *
 * DO NOT "fix" anything in here. Its value is that it is unchanged.
 */
...
export const computeOutgoingConditionsEi = (payload: {
    players: any[];
    targets: any[];
